...
class DictTypeService:
    ...
    @classmethod
    def get_all_dict_type_services(cls, query_db: Session):
        """
        获取字所有典类型列表信息service
        :param query_db: orm对象
        :return: 字典类型列表信息对象
        """
        dict_type_list_result = DictTypeDao.get_all_dict_type(query_db)
    ...
    async def add_dict_type_services(cls, request: Request, query_db: Session, page_object: DictTypeModel):
        ...
        :return: 新增字典类型校验结果
        """
        dict_type = DictTypeDao.get_dict_type_detail_by_info(query_db, DictTypeModel(**dict(dict_type=page_object.dict_type)))
        if dict_type:
    ...
    async def edit_dict_type_services(cls, request: Request, query_db: Session, page_object: DictTypeModel):
        ...
        dict_type_info = cls.detail_dict_type_services(query_db, edit_dict_type.get('dict_id'))
        if dict_type_info:
            if dict_type_info.dict_type != page_object.dict_type or dict_type_info.dict_name != page_object.dict_name:
                dict_type = DictTypeDao.get_dict_type_detail_by_info(query_db, DictTypeModel(
        ...
        if page_object.dict_ids.split(','):
            ...
            try:
                for dict_id in dict_id_list:
                    ...
                    DictTypeDao.delete_dict_type_dao(query_db, DictTypeModel(**dict_id_dict))
                query_db.commit()
                await DictDataService.init_cache_sys_dict_services(query_db, request.app.state.redis)
    ...
    @classmethod
    def detail_dict_type_services(cls, query_db: Session, dict_id: int):
        """
        获取字典类型详细信息service
        :param query_db: orm对象
...
class DictDataService:
    ...
    async def edit_dict_data_services(cls, request: Request, query_db: Session, page_object: DictDataModel):
        ...
        dict_data_info = cls.detail_dict_data_services(query_db, edit_data_type.get('dict_code'))
        if dict_data_info:
        ...
        if page_object.dict_codes.split(','):
            ...
            try:
                for dict_code in dict_code_list:
                    ...
                    DictDataDao.delete_dict_data_dao(query_db, DictDataModel(**dict_code_dict))
                query_db.commit()
                await cls.init_cache_sys_dict_services(query_db, request.app.state.redis)
                result = dict(is_success=True, message='删除成功')
            except Exception as e:
    ...
    @classmethod
    def detail_dict_data_services(cls, query_db: Session, dict_code: int):
        """
        获取字典数据详细信息service
        :param query_db: orm对象
