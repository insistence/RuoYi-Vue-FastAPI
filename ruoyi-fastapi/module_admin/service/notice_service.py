...
class NoticeService:
    ...
    def edit_notice_services(cls, query_db: Session, page_object: NoticeModel):
        ...
        notice_info = cls.detail_notice_services(query_db, edit_notice.get('notice_id'))
        if notice_info:
        ...
        if page_object.notice_ids.split(','):
            ...
            try:
                for notice_id in notice_id_list:
                    ...
                    NoticeDao.delete_notice_dao(query_db, NoticeModel(**notice_id_dict))
                query_db.commit()
    ...
    @classmethod
    def detail_notice_services(cls, query_db: Session, notice_id: int):
        """
        获取通知公告详细信息service
        :param query_db: orm对象
