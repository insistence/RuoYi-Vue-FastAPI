...
// Vue 2 共享响应状态，账号时区变更后页面无需刷新。
const timezoneState = Vue.observable({
...
export function setBusinessTimezone(timezoneName) {
  ...
  timezoneState.businessTimezone = timezoneName.trim()
}
...
export function getBusinessTimezone() {
  return timezoneState.businessTimezone
}
...
export function setUserTimezone(preference = 'auto') {
  ...
  timezoneState.userTimezone = preference.trim()
...
export function getUserTimezone() {
  return timezoneState.userTimezone
}
...
export function refreshDeviceTimezone() {
  try {
    ...
    if (Intl.DateTimeFormat.polyfilled) {
      timezoneState.deviceTimezone = null
      return null
    }
    const name = new Intl.DateTimeFormat().resolvedOptions().timeZone
    getFormatter(name)
    timezoneState.deviceTimezone = name
  } catch {
    timezoneState.deviceTimezone = null
  }
  return timezoneState.deviceTimezone
}
...
export function getDisplayTimezone() {
  return timezoneState.userTimezone === 'auto' ? timezoneState.deviceTimezone || timezoneState.businessTimezone : timezoneState.userTimezone
}
...
setBusinessTimezone(timezoneState.businessTimezone)
refreshDeviceTimezone()
...
async function chooseTimeOffset(error) {
  ...
  const { value } = await ElementUI.MessageBox.prompt(
