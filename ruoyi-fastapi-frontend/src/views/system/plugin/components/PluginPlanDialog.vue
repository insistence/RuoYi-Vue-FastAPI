   ...
   <el-dialog :title="title" :visible="visible" width="920px" append-to-body @close="emit('update:visible', false)">
         ...
         <el-tab-pane label="计划项">
            <el-table :data="planResult.items" size="small" border empty-text="暂无计划项">
               ...
               <el-table-column label="阻塞项" width="90" align="center">
                  <template slot-scope="scope">
                     <el-tag :type="getBlockerCount(scope.row) ? 'danger' : 'success'">{{ getBlockerCount(scope.row) }}</el-tag>
                  </template>
               </el-table-column>
            </el-table>
         </el-tab-pane>
         ...
         <el-tab-pane label="执行结果">
            ...
            <el-table :data="batchResult.executed" size="small" border empty-text="暂无执行记录">
               ...
               <el-table-column label="耗时" prop="durationMs" width="90" align="center">
                  <template slot-scope="scope">{{ formatDuration(scope.row.durationMs) }}ms</template>
               </el-table-column>
      ...
      <div slot="footer" class="dialog-footer">
         <el-switch
            ...
            inactive-text="失败即中止"
            class="mr12"
            @input="emit('update:continueOnError', $event)"
         />
         <el-button
            type="primary"
