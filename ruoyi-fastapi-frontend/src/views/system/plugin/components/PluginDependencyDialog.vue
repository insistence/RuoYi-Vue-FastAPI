<template>
   <el-dialog title="插件依赖" :visible="visible" width="860px" append-to-body @close="emit('update:visible', false)">
      <el-alert
         v-if="result.message"
      ...
      <div class="dependency-summary">
         ...
         <div class="dependency-summary-item">
            ...
            <span class="dependency-summary-value">{{ formatPlanCount(result) }}</span>
         </div>
         <div class="dependency-summary-item">
         ...
         <el-tab-pane label="依赖状态">
            <el-table :data="result.dependencies || []" size="small" border empty-text="暂无依赖声明">
               ...
               <el-table-column label="已安装版本" min-width="130" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ scope.row.installedVersion || "-" }}</template>
               </el-table-column>
         ...
         <el-tab-pane v-if="policy" label="策略判定">
            ...
            <el-table :data="policy.items || []" size="small" border empty-text="暂无策略项">
               ...
               <el-table-column label="锁定版本" min-width="120" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ scope.row.lockedVersion || "-" }}</template>
               </el-table-column>
               <el-table-column label="离线制品" min-width="220" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ scope.row.artifactPath || "-" }}</template>
               </el-table-column>
               <el-table-column label="说明" min-width="260" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ formatPolicyItemMessage(scope.row) }}</template>
               </el-table-column>
            </el-table>
         </el-tab-pane>
      </el-tabs>
...
const emit = defineEmits(["update:visible", "dry-run", "install"]);
...
function formatPlanCount(result) {
  return result.planCount === null || result.planCount === undefined
