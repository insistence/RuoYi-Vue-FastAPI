<template>
   <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8">
         <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-share" :disabled="!selectedPluginIds.length" @click="handlePlan('install')" v-hasPermi="['system:plugin:query']">安装计划</el-button>
         </el-col>
         <el-col :span="1.5">
            <el-button type="success" plain icon="el-icon-connection" :disabled="!selectedPluginIds.length" @click="handlePlan('enable')" v-hasPermi="['system:plugin:query']">启用计划</el-button>
         </el-col>
         <el-col :span="1.5">
            <el-button type="warning" plain icon="el-icon-upload2" :disabled="!selectedPluginIds.length" @click="handlePlan('upgrade')" v-hasPermi="['system:plugin:query']">升级计划</el-button>
         </el-col>
         <el-col :span="1.5">
            <el-button plain icon="el-icon-tickets" @click="handleOperationLog" v-hasPermi="['system:plugin:query']">审计记录</el-button>
         </el-col>
         <el-col :span="1.5">
            ...
            <template slot-scope="scope">
               <div class="plugin-action-buttons">
                  <el-tooltip content="详情" placement="top">
                     <el-button type="text" icon="el-icon-view" @click="handleDetail(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip content="配置" placement="top" v-if="!isOrphanPlugin(scope.row)">
                     <el-button type="text" icon="el-icon-setting" @click="handleConfig(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip content="依赖" placement="top" v-if="!isOrphanPlugin(scope.row)">
                     <el-button type="text" icon="el-icon-connection" @click="handleDependencies(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip content="检查" placement="top" v-if="!isOrphanPlugin(scope.row)">
                     <el-button type="text" icon="el-icon-circle-check" @click="handleCheck(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip content="健康检查" placement="top" v-if="!isOrphanPlugin(scope.row)">
                     <el-button type="text" icon="el-icon-first-aid-kit" @click="handleHealth(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip content="诊断包" placement="top" v-if="!isOrphanPlugin(scope.row)">
                     <el-button type="text" icon="el-icon-document-checked" @click="handleDiagnose(scope.row)" v-hasPermi="['system:plugin:query']" />
                  </el-tooltip>
                  <el-tooltip :content="getOperationTooltip(scope.row, 'install', '安装')" placement="top" v-if="canInstall(scope.row)">
                     <el-button type="text" icon="el-icon-download" :disabled="isOperationBlocked(scope.row, 'install')" @click="handleInstallDryRun(scope.row)" v-hasPermi="['system:plugin:edit']" />
                  </el-tooltip>
                  <el-tooltip :content="getOperationTooltip(scope.row, 'upgrade', '升级')" placement="top" v-if="canUpgrade(scope.row)">
                     <el-button type="text" icon="el-icon-upload2" :disabled="isOperationBlocked(scope.row, 'upgrade')" @click="handleUpgradeDryRun(scope.row)" v-hasPermi="['system:plugin:edit']" />
                  </el-tooltip>
                  <el-tooltip :content="getOperationTooltip(scope.row, 'uninstall', '卸载')" placement="top" v-if="canUninstall(scope.row)">
                     <el-button type="text" class="plugin-action-danger" icon="el-icon-switch-button" :disabled="isOperationBlocked(scope.row, 'uninstall')" @click="handleUninstallDryRun(scope.row)" v-hasPermi="['system:plugin:edit']" />
                  </el-tooltip>
                  <el-tooltip content="清理孤儿元数据" placement="top" v-if="isOrphanPlugin(scope.row)">
                     <el-button type="text" class="plugin-action-danger" icon="el-icon-delete" @click="handlePurgeDryRun(scope.row)" v-hasPermi="['system:plugin:remove']" />
                  </el-tooltip>
               </div>
            </template>
         </el-table-column>
      ...
      <plugin-plan-dialog
         ...
         :continue-on-error.sync="batchContinueOnError"
         :title="planTitle"
      ...
      <el-drawer title="插件操作审计" :visible.sync="operationLogOpen" size="50%" append-to-body>
         <div class="plugin-audit-drawer">
               ...
               <el-form :model="operationLogQueryParams" :inline="true" label-width="68px" class="plugin-query-form plugin-log-toolbar">
                  <el-form-item label="插件ID">
                     <el-input
                        ...
                        placeholder="请输入插件ID"
                        clearable
                        @keyup.enter.native="handleOperationLogQuery"
                     />
                  </el-form-item>
                  ...
                  <el-form-item>
                     <el-button type="primary" icon="el-icon-search" @click="handleOperationLogQuery">搜索</el-button>
                     <el-button icon="el-icon-refresh" @click="resetOperationLogQuery">重置</el-button>
                  </el-form-item>
               </el-form>
            </div>
            ...
            <el-collapse v-model="operationLogMaintenanceActive" class="plugin-audit-maintenance">
               <el-collapse-item name="retention">
                  ...
                  <el-form :model="operationLogRetentionForm" :inline="true" label-width="84px" class="plugin-log-toolbar">
                     ...
                     <el-form-item>
                        <el-button icon="el-icon-document-checked" :loading="operationLogRetentionLoading" @click="handleOperationLogRetentionPreview">预览清理</el-button>
                        <el-button
                           type="danger"
                           plain
                           icon="el-icon-delete"
            ...
            <div class="plugin-audit-table">
               <div class="plugin-audit-table-toolbar">
                  ...
                  <right-toolbar :showSearch.sync="operationLogShowSearch" @queryTable="getOperationLogList"></right-toolbar>
               </div>
               <el-table v-loading="operationLogLoading" :data="operationLogList" size="small" border>
                  ...
                  <el-table-column label="操作" width="90" align="center">
                     <template slot-scope="scope">{{ formatPluginOperation(scope.row.operation) }}</template>
                  </el-table-column>
                  ...
                  <el-table-column label="插件" min-width="180" :show-overflow-tooltip="true">
                     <template slot-scope="scope">{{ formatPlanDependencies(scope.row.pluginIds) }}</template>
                  </el-table-column>
                  <el-table-column label="成功" width="70" align="center">
                     <template slot-scope="scope">{{ getSummaryCount(scope.row, "succeeded") }}</template>
                  </el-table-column>
                  <el-table-column label="失败" width="70" align="center">
                     <template slot-scope="scope">{{ getSummaryCount(scope.row, "failed") }}</template>
                  </el-table-column>
                  ...
                  <el-table-column label="操作" width="70" align="center">
                     <template slot-scope="scope">
                        <el-tooltip content="详情" placement="top">
                           <el-button type="text" icon="el-icon-view" @click="handleOperationLogDetail(scope.row)" />
                        </el-tooltip>
                     </template>
                  </el-table-column>
               </el-table>
      ...
      <el-dialog title="审计详情" :visible.sync="operationLogDetailOpen" width="920px" append-to-body>
         ...
         <el-tabs>
            <el-tab-pane label="摘要">
               <el-descriptions :column="3" border class="mb16">
                  <el-descriptions-item label="成功数">{{ getSummaryCount(operationLogDetail, "succeeded") }}</el-descriptions-item>
                  <el-descriptions-item label="失败数">{{ getSummaryCount(operationLogDetail, "failed") }}</el-descriptions-item>
                  <el-descriptions-item label="跳过数">{{ getSummaryCount(operationLogDetail, "skipped") }}</el-descriptions-item>
                  <el-descriptions-item label="变更配置" :span="3">
                     {{ formatPlanDependencies(getSummaryChangedKeys(operationLogDetail)) }}
                  </el-descriptions-item>
                  <el-descriptions-item label="失败建议" :span="3">
                     {{ operationLogDetail.failedSuggestion || "暂无" }}
                  </el-descriptions-item>
                  <el-descriptions-item label="失败步骤" :span="3">
                     {{ getOperationFailedStep(operationLogDetail) }}
                  </el-descriptions-item>
               </el-descriptions>
            </el-tab-pane>
            ...
            <el-tab-pane label="动作计划">
               <el-table :data="operationLogDetail.actionItems || []" size="small" border empty-text="暂无动作计划">
                  ...
                  <el-table-column label="数量" prop="count" width="80" align="center">
                     <template slot-scope="scope">{{ formatNullable(scope.row.count) }}</template>
                  </el-table-column>
                  <el-table-column label="状态" min-width="160" :show-overflow-tooltip="true">
                     <template slot-scope="scope">{{ formatActionPlanState(scope.row) }}</template>
                  </el-table-column>
               </el-table>
            </el-tab-pane>
            <el-tab-pane label="依赖计划">
               <el-table :data="operationLogDetail.planItems || []" size="small" border empty-text="暂无依赖计划">
                  ...
                  <el-table-column label="依赖" min-width="200" :show-overflow-tooltip="true">
                     <template slot-scope="scope">{{ formatPlanDependencies(scope.row.dependencies) }}</template>
                  </el-table-column>
               </el-table>
            </el-tab-pane>
            <el-tab-pane label="执行记录">
               <el-table :data="getExecutedResults(operationLogDetail)" size="small" border empty-text="暂无执行记录">
                  ...
                  <el-table-column label="耗时" prop="durationMs" width="90" align="center">
                     <template slot-scope="scope">{{ formatNullable(scope.row.durationMs) }}ms</template>
                  </el-table-column>
                  <el-table-column label="失败步骤" prop="failedStep" width="150" :show-overflow-tooltip="true">
                     <template slot-scope="scope">{{ getExecutedFailedStep(scope.row) }}</template>
                  </el-table-column>
                  <el-table-column label="说明" prop="message" min-width="220" :show-overflow-tooltip="true" />
            ...
            <el-tab-pane label="原始结果">
               <el-input :value="formatJson(operationLogDetail.result)" type="textarea" :rows="14" readonly />
            </el-tab-pane>
         </el-tabs>
      ...
      <el-dialog :title="actionTitle" :visible.sync="actionOpen" width="860px" append-to-body>
         <el-alert
         ...
         <el-tabs>
            <el-tab-pane v-if="actionResult.error" label="错误详情">
               ...
               <el-input :value="formatJson(actionResult.error.raw)" type="textarea" :rows="10" readonly />
            </el-tab-pane>
            <el-tab-pane label="动作计划">
...
function getSummaryCount(payload, key) {
  const value = payload && payload.summary ? payload.summary[key] : undefined;
  return value === undefined || value === null ? 0 : value;
}

function getSummaryChangedKeys(payload) {
  return payload && payload.summary && Array.isArray(payload.summary.changedKeys)
    ? payload.summary.changedKeys
    : [];
}

function getOperationFailedStep(payload) {
  if (!payload) {
    return "暂无";
  }
  const result = payload.result || {};
  const failed = result.failed || {};
  return payload.failedStep || result.failedStep || (failed.result && failed.result.failedStep) || "暂无";
}

function getExecutedResults(payload) {
  const result = payload && payload.result;
  return result && Array.isArray(result.executed) ? result.executed : [];
}

function getExecutedFailedStep(row) {
  return row.failedStep || (row.result && row.result.failedStep) || "-";
}
...
.plugin-action-buttons ::v-deep .el-button {
  margin-left: 0;
}

.plugin-query-form ::v-deep .el-input,
.plugin-query-form ::v-deep .el-select {
  width: 220px;
}
...
.plugin-audit-maintenance ::v-deep .el-collapse-item__header {
  height: 40px;
  padding: 0 12px;
...
.plugin-audit-maintenance ::v-deep .el-collapse-item__wrap {
  background: transparent;
  border-bottom: 0;
}

.plugin-audit-maintenance ::v-deep .el-collapse-item__content {
  padding: 0 12px 12px;
}
...
.plugin-log-toolbar ::v-deep .el-form-item {
  margin-bottom: 10px;
}

.plugin-audit-date-range ::v-deep .el-date-editor {
  width: 220px;
}
