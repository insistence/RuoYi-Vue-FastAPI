...
class ConfigService:
    ...
    async def add_config_services(cls, request: Request, query_db: Session, page_object: ConfigModel):
        ...
        :return: 新增参数配置校验结果
        """
        config = ConfigDao.get_config_detail_by_info(query_db, ConfigModel(**dict(config_key=page_object.config_key)))
        if config:
    ...
    async def edit_config_services(cls, request: Request, query_db: Session, page_object: ConfigModel):
        ...
        config_info = cls.detail_config_services(query_db, edit_config.get('config_id'))
        if config_info:
        ...
        if page_object.config_ids.split(','):
            ...
            try:
                for config_id in config_id_list:
                    ...
                    ConfigDao.delete_config_dao(query_db, ConfigModel(**config_id_dict))
                query_db.commit()
                await cls.init_cache_sys_config_services(query_db, request.app.state.redis)
    ...
    @classmethod
    def detail_config_services(cls, query_db: Session, config_id: int):
        """
        获取参数配置详细信息service
        :param query_db: orm对象
