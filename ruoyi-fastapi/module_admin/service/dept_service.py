...
class DeptService:
    ...
    @classmethod
    def get_dept_tree_for_edit_option_services(cls, query_db: Session, page_object: DeptModel, data_scope_sql: str):
        """
        获取部门编辑部门树信息service
        :param query_db: orm对象
        :param page_object: 查询参数对象
    ...
    def add_dept_services(cls, query_db: Session, page_object: DeptModel):
        ...
        dept = DeptDao.get_dept_detail_by_info(query_db, DeptModel(**dict(parent_id=page_object.parent_id, dept_name=page_object.dept_name)))
        if dept:
    ...
    def edit_dept_services(cls, query_db: Session, page_object: DeptModel):
        ...
        dept_info = cls.detail_dept_services(query_db, edit_dept.get('dept_id'))
        if dept_info:
            if dept_info.parent_id != page_object.parent_id or dept_info.dept_name != page_object.dept_name:
                dept = DeptDao.get_dept_detail_by_info(query_db, DeptModel(
            ...
            try:
                ...
                cls.update_children_info(query_db, DeptModel(dept_id=page_object.dept_id,
        ...
        if page_object.dept_ids.split(','):
            ...
            try:
                for dept_id in dept_id_list:
                    ...
                    DeptDao.delete_dept_dao(query_db, DeptModel(**dept_id_dict))
                query_db.commit()
    ...
    @classmethod
    def detail_dept_services(cls, query_db: Session, dept_id: int):
        """
        获取部门详细信息service
        :param query_db: orm对象
        :param dept_id: 部门id
