...
class JobService:
    ...
    def edit_job_services(cls, query_db: Session, page_object: EditJobModel):
        ...
        if page_object.type == 'status':
            del edit_job['type']
        job_info = cls.detail_job_services(query_db, edit_job.get('job_id'))
        if job_info:
    ...
    def execute_job_once_services(cls, query_db: Session, page_object: JobModel):
        ...
        job_info = cls.detail_job_services(query_db, page_object.job_id)
        if job_info:
        ...
        if page_object.job_ids.split(','):
            ...
            try:
                for job_id in job_id_list:
                    ...
                    JobDao.delete_job_dao(query_db, JobModel(**job_id_dict))
                query_db.commit()
    ...
    @classmethod
    def detail_job_services(cls, query_db: Session, job_id: int):
        """
        获取定时任务详细信息service
        :param query_db: orm对象
        :param job_id: 定时任务id
