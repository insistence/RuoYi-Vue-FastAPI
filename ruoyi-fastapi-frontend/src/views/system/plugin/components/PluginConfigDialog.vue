<template>
   <el-dialog :title="title" :visible="visible" width="720px" append-to-body @close="emit('update:visible', false)">
            ...
            >
               ...
               <template v-else-if="item.type === 'textarea' || item.type === 'json'">
                  ...
                  <div v-if="item.type === 'json'" class="config-inline-actions">
                     <el-button type="text" icon="el-icon-magic-stick" @click="formatConfigJsonValue(item)">格式化 JSON</el-button>
                  </div>
               </template>
               <el-input
                  v-else
