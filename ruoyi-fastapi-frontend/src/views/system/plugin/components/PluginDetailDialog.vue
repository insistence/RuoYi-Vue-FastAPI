<template>
   <el-dialog title="插件详情" :visible="visible" width="920px" append-to-body @close="emit('update:visible', false)">
      <el-tabs>
         <el-tab-pane label="概览">
            ...
            <el-descriptions :column="2" border class="mb16">
               ...
               <el-descriptions-item label="分类">{{ metadata.category || "-" }}</el-descriptions-item>
               <el-descriptions-item label="标签">{{ formatDetailTags(metadata.tags) }}</el-descriptions-item>
            ...
            <el-descriptions :column="2" border class="mb16">
               ...
               <el-descriptions-item label="后端模块" :span="2">{{ backend.module || "-" }}</el-descriptions-item>
               <el-descriptions-item label="自动扫描路由">{{ formatYesNo(backend.autoScanRouters) }}</el-descriptions-item>
               <el-descriptions-item label="定时任务">{{ backendJobs.length }}</el-descriptions-item>
            </el-descriptions>
            ...
            <el-descriptions :column="2" border>
               ...
               <el-descriptions-item label="前端插件ID">{{ frontend.pluginId || "-" }}</el-descriptions-item>
               <el-descriptions-item label="前端基础路径">{{ frontend.basePath || "-" }}</el-descriptions-item>
               <el-descriptions-item label="视图目录">{{ frontend.viewsPath || "-" }}</el-descriptions-item>
               <el-descriptions-item label="API 目录">{{ frontend.apiPath || "-" }}</el-descriptions-item>
               <el-descriptions-item label="前端交付">{{ getFrontendDeliveryLabel(frontend.delivery) }}</el-descriptions-item>
               <el-descriptions-item label="菜单声明">{{ frontendMenus.length }}</el-descriptions-item>
            </el-descriptions>
         </el-tab-pane>
         <el-tab-pane label="菜单">
            <el-table :data="frontendMenus" size="small" border empty-text="暂无菜单声明">
         ...
         <el-tab-pane label="配置">
            <el-table :data="detail.config || []" size="small" border empty-text="暂无配置声明">
               ...
               <el-table-column label="默认值" min-width="160" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ formatConfigDefaultValue(scope.row) }}</template>
               </el-table-column>
               <el-table-column label="约束" min-width="200" :show-overflow-tooltip="true">
                  <template slot-scope="scope">{{ formatConfigConstraint(scope.row) }}</template>
               </el-table-column>
               <el-table-column label="说明" prop="description" min-width="220" :show-overflow-tooltip="true" />
            </el-table>
         </el-tab-pane>
         <el-tab-pane label="依赖">
            ...
            <el-table v-loading="migrationLoading" :data="migrationHistory" size="small" border empty-text="暂无 migration 执行历史">
               ...
               <el-table-column label="次数" prop="attemptCount" width="70" align="center">
                  <template slot-scope="scope">{{ formatCount(scope.row.attemptCount) }}</template>
               </el-table-column>
               <el-table-column label="语句" prop="statementCount" width="70" align="center">
                  <template slot-scope="scope">{{ formatCount(scope.row.statementCount) }}</template>
               </el-table-column>
               <el-table-column label="开始时间" width="160" align="center">
                  <template slot-scope="scope">{{ formatPluginTime(scope.row.startedTime || scope.row.createTime) }}</template>
               </el-table-column>
               <el-table-column label="结束时间" width="160" align="center">
                  <template slot-scope="scope">{{ formatPluginTime(scope.row.finishedTime) }}</template>
               </el-table-column>
            ...
            <el-table :data="backendJobs" size="small" border empty-text="暂无定时任务声明">
...
const emit = defineEmits(["update:visible", "repair", "mark-migration-success", "mark-migration-failed"]);

const metadata = computed(() => props.detail.metadata || {});
const backend = computed(() => props.detail.backend || {});
const frontend = computed(() => props.detail.frontend || {});
const frontendMenus = computed(() => Array.isArray(frontend.value.menus) ? frontend.value.menus : []);
