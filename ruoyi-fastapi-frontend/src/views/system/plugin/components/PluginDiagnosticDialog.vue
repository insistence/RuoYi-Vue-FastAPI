<template>
   <el-dialog :title="title" :visible="visible" width="920px" append-to-body @close="emit('update:visible', false)">
      <el-alert
         v-if="result.message"
      ...
      <template v-else>
         <div class="diagnostic-summary">
            ...
            <div v-if="health" class="diagnostic-summary-item">
               ...
               <span class="diagnostic-summary-value">{{ formatDuration(health.durationMs) }}ms</span>
            </div>
         </div>
         ...
         <el-tabs>
            <el-tab-pane v-if="health" label="健康状态">
               ...
               <el-input :value="formatJson(health.details)" type="textarea" :rows="8" readonly />
            </el-tab-pane>

            <el-tab-pane v-if="diagnose" label="检查摘要">
               <el-descriptions :column="3" border class="mb16">
                  <el-descriptions-item label="依赖">{{ formatBoolean(diagnoseCheck.dependencyOk) }}</el-descriptions-item>
                  <el-descriptions-item label="Manifest">{{ formatBoolean(diagnoseCheck.manifestOk) }}</el-descriptions-item>
                  <el-descriptions-item label="结构">{{ formatBoolean(diagnoseCheck.structureOk) }}</el-descriptions-item>
                  <el-descriptions-item label="插件依赖">{{ formatBoolean(diagnoseCheck.pluginDependencyOk) }}</el-descriptions-item>
                  <el-descriptions-item label="菜单冲突">{{ formatBoolean(diagnoseCheck.menuConflictOk) }}</el-descriptions-item>
                  <el-descriptions-item label="配置项">{{ diagnoseConfigTotal }}</el-descriptions-item>
               </el-descriptions>
            ...
            <el-tab-pane v-if="diagnose" label="菜单计划">
               <el-input :value="formatJson(diagnose.menuPlan)" type="textarea" :rows="12" readonly />
            </el-tab-pane>

            <el-tab-pane label="原始数据">
               <el-input :value="formatJson(result)" type="textarea" :rows="14" readonly />
            </el-tab-pane>
         </el-tabs>
      </template>
...
const diagnoseCheck = computed(() => diagnose.value && diagnose.value.check ? diagnose.value.check : {});
const diagnoseConfigTotal = computed(() => {
  const config = diagnose.value && diagnose.value.config;
  const total = config && config.summary ? config.summary.total : undefined;
